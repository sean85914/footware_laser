...
from geometry_msgs.msg import Point, Pose, Twist, Vector3, Quaternion, PoseArray
...
array_pub = rospy.Publisher('/pixel_array', PoseArray, queue_size = 10)
...
def rosHSVProcessCallBack(msg):
    ...
    img_crop = np.zeros((640, 480)).astype(np.uint8)
    res = cv2.cvtColor(res, cv2.COLOR_BGR2GRAY)
    for i in range(110,430):
        for j in range(480):
             	img_crop[i][j]=res[i][j]
    
    #res_eroded = cv2.erode(img_crop, None, iterations = 2)
    res_dilated = cv2.dilate(img_crop, None, iterations = 4)
    res_dilated = cv2.erode(res_dilated, None, iterations = 6)
    contours, hierarchy = cv2.findContours(res_dilated, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
	
    img_crop = cv2.cvtColor(img_crop, cv2.COLOR_GRAY2BGR)
    cv2.drawContours(img_crop, contours, -1, (0, 0, 255), 5)
    if len(contours) != 1:
        num_of_point_list = []
        for i in range(len(contours)):
			num_of_point_list.append(len(contours[i]))
        max_idx = np.argmax(num_of_point_list)
    else:
        max_idx = 0
    arr = PoseArray()
    for i in range(len(contours[max_idx])):
        if i%3 == 0:
            #print contours[max_idx][i][0][0]
            p = Pose()
            p.position.x = contours[max_idx][i][0][0]
            p.position.y = contours[max_idx][i][0][1]
            arr.poses.append(p)
    print "There are total ", len(arr.poses), " points"
    array_pub.publish(arr)
    #print "End contours with ", len(contours[max_idx]), " points"
    img_result_pub.publish(cv_bridge.cv2_to_imgmsg(img_crop, encoding="passthrough"))
def HSVObjectDetection(cv_image, toPrint = True):
    ...
    lower_green = np.array([77,43,46])
    upper_green = np.array([101,255,245])

    # Threshold the HSV image to get only red colors
    mask = cv2.inRange(hsv_image, lower_green, upper_green)
    #mask = mask.astype(np.uint8)
    #mask = mask[0:400, 160:400]
    ...
    for cnt in contours:
        if not draw_contours:
            ...
            if detect_shape:
                ...
                shape = cv2.approxPolyDP(cnt, 0.04 * peri, True)
...
def get_side_length(cnt_vtx_list, depthimage):
    ...
    for i in range(0, len(vtx_3d_list)):
        if vtx_3d_list[i] == None:
            return None
    if len(vtx_3d_list) > 1:
        for i in range(0 , len(vtx_3d_list)-1):
            vector = vtx_3d_list[i] - vtx_3d_list[i+1]
            side_len = math.sqrt(sum(j**2 for j in vector))
...
def get_radius(cnt_vtx_list, depthimage):
    ...
    for i in range(0, len(vtx_3d_list)):
        if vtx_3d_list[i] == None:
            return None
    if len(vtx_3d_list) > 0:
        for i in range(0, len(vtx_3d_list)):
            for j in range(0, len(vtx_3d_list)):
                vector = vtx_3d_list[i] - vtx_3d_list[j]
                vector_len = math.sqrt(sum(k**2 for k in vector))
                vtx_dis.append(vector_len) 
    # Example for calculating distance:
    # vector = vtx_3d_list[i] - vtx_3d_list[i+1]
    # side_len = math/sqrt(i**2 for i in vector)
    #
